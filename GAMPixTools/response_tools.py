...
def apply_detector_response(events):
    ...
    def add_values_to_cells(values, cell_values, hits, nh, event_cut):
        ...
        for nc in range(hits['num_cells'].max()):
            hit_in_cell \
                = hits['hit_cell'][nh, :] == hits['cells_list'][nc, :]
    ...
    #   convenient variable
    max_num_hits = events.truth_hits['num_hits'].max()
    ...
    measured_hits['total_energy'] \
        = np.zeros(events.truth_hits['num_hits'].shape)
    ...
    for nh in range(max_num_hits):
        ...
        #   convenient variables
        alive = events.truth_hits['alive'][nh, :]
        i_cells = events.truth_hits['hit_cell_index'][nh, :]
    ...
    measured_hits['hit_cell'] = copy.copy(events.truth_hits['hit_cell'])
    measured_hits['hit_cell_index'] \
        = copy.copy(events.truth_hits['hit_cell_index'])
            ...
            while 1:
                ...
                if np.sum(shift)>0:
                    ...
                    for n2 in range(n1,max_num_hits-1):
                        ...
                        measured_hits['hit_cell'][n2, shift] = \
                            measured_hits['hit_cell'][n2+1, shift]
                        measured_hits['hit_cell_index'][n2, shift] = \
                            measured_hits['hit_cell_index'][n2+1, shift]
                    ...
                    measured_hits['hit_cell'][n2+1, shift] = 0
                    measured_hits['hit_cell_index'][n2+1, shift] = 0
    ...
    if measured_hits['alive'].shape[0] > max_hits:
        ...
        measured_hits['hit_cell'] = measured_hits['hit_cell'][0:max_hits, :]
        measured_hits['hit_cell_index'] \
            = measured_hits['hit_cell_index'][0:max_hits, :]
