...
def global_to_cell_coordinates(
    ...
    alive and hit_cell have dimension [event]
    hit_cell is cell id for each entry

    r_in, hit_cell and alive must be numpy arrays
    """
    ...
    if r_in.ndim==3:
        ...
        cell_p = np.expand_dims(hit_cell, axis=1)

    #   Set alive to True if not supplied
    if not 'alive' in locals():
