...
def readout_dual_scale_pixels(r, num_e, array_sensors, pixel_sensors,
    ...
    for n in range(voxels['voxel_indices'].shape[1]):
        ...
        chip_samples = charge_readout_tools.readout_pixels(
            r_chip,
            num_e_chip,
            pixel_sensors,
...
def readout_pixels(r, num_e, pixel_sensors, z_limits=None,
    ...
    if raw_output:
        ...
        location_mask = locatoins_z < 0
        mask = signal_mask & location_mask

        samples_raw = samples_raw[mask].flatten()
        r_raw = np.zeros((3, np.sum(mask)))
        r_raw[0, :] = locations_x[mask]
        r_raw[1, :] = locations_y[mask]
        r_raw[2, :] = locations_z[mask]
