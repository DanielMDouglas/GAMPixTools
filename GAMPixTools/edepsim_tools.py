...
#inputs should be events where the primary vertex is at 0 (this is the case for neutrinos coming from GENIE sims).
#outputs randomize the primary vertex location within position_bounds and randomize the primary momentum).
import numpy as np
from scipy.spatial.transform import Rotation as R

def randomize_primary_state(segments, trajectories, position_bounds):
    # Generate a single random displacement and rotation
    random_displacement = np.array([np.random.uniform(low, high) for low, high in position_bounds])
    random_rotation = R.random().as_matrix()

    # Apply the random displacement and rotation to all position and momentum attributes
    position_attributes = [('x_start', 0), ('y_start', 1), ('z_start', 2),
                           ('x_end', 0), ('y_end', 1), ('z_end', 2),
                           ('x', 0), ('y', 1), ('z', 2)]
    momentum_attributes = ['pxyz_start', 'pxyz_end']

    for attr, index in position_attributes:
        if attr in segments.dtype.names:
            segments[attr] += random_displacement[index]
  
    for attr in momentum_attributes:
        trajectories[attr] = (random_rotation @ trajectories[attr].T).T
            
    return segments, trajectories



def quench(tracks, mode = consts.BIRKS):
    """
    ...
    Args:
        ...
        mode (int): recombination model (consts.BOX or consts.BIRKS).
    """
    # disabling tqdm bar for now
